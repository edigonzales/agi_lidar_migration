...
def DATA_FOLDER = USER_HOME + "/tmp/ch.so.agi.lidar_2025.dtm.gpkg_tmp/"
def TINDEX = "../data/2025/tindex.shp"
def TEMP_FOLDER = USER_HOME + "/tmp/contours/"
def XTF_FOLDER = USER_HOME + "/tmp/ch.so.agi.lidar_2025_contour0_5m.xtf/"
def GPKG_FOLDER = USER_HOME + "/tmp/ch.so.agi.lidar_2025_contour0_5m.gpkg/"
def DXF_FOLDER = USER_HOME + "/tmp/ch.so.agi.lidar_2025_contour0_5m.dxf/"
...
GParsPool.withPool(1) {
    ...
    features.each {f ->
        try {
            ...
            tile = tileX + "-" + tileY;
            tileGpkg = tileX + "000_" + tileY + "000";
            tileResult = tileGpkg + ".ch.swisstopo.dtm_2025.hoehenlinien"

            if(!tile.equalsIgnoreCase("2593-1226")) {
                return
            }
            ...
            GeoPackage workspace = new GeoPackage(Paths.get(tmpDir.getAbsolutePath(), tileGpkg + ".gpkg").toFile().getAbsolutePath())
            ...
            def xtfZipFileName = Paths.get(XTF_FOLDER, tileResult + ".xtf.zip")
            ...
            def gpkgZipFileName = Paths.get(GPKG_FOLDER, tileResult + ".gpkg.zip")
            ...
            def dxfZipFileName = Paths.get(DXF_FOLDER, tileResult + ".dxf.zip")
